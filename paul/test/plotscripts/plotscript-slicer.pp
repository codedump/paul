...
def init(canvas):
    ...
    GUI.layout.setContentsMargins (-1, -1, 0, 0)
    ...
    GUI.layout.addWidget (GUI.slicewin, 0, 2, 3, 2)

    #PLOT.canvas = canvas
    #PLOT.waves = []
    #PLOT.canvas.axes = PLOT.canvas.fig.add_subplot (111)
    #PLOT.canvas.axes.hold(False)
    ##PLOT.canvas.axes.imshow ([[0, 0], [0, 0]])
    #PLOT.canvas.fig.colorbar(PLOT.canvas.axes.images[0])
...
#
#    can.axes.clear()
#    can.axes.imshow (wav[0], aspect='auto', extent=wav[0].imgLim())
#    decorate (can, wav)
